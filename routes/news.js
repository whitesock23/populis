const express = require("express");
const router = express.Router();
const Deputy = require("../models/Deputy.model");
const axios = require("axios");
const deputiesAPI = "https://raw.githubusercontent.com/centraldedados/parlamento-deputados/master/data/deputados.json"
const parliamentAPI = "https://app.parlamento.pt/webutils/docs/doc.txt?path=&fich=InformacaoBaseXIV_json.txt"

//
router.get("/news", async(req, res) => {
    const response = await axios.get(parliamentAPI);
    const news = response.data;
    console.log(news);
    res.render("news", { layout: false, news });
});

// router.get("/news", async (req, res, next) => {
//   try {
//     const response = await axios.get(parliamentAPI);
//     const news = response.data;
//     console.log(news)
//     res.render("news", {layout: false, news });
//   } catch (err) {
//     next(err);
//   }
// });

//not working
router.get("/news/:newsId", async(req, res, next) => {
    try {
        const findnewsId = findById(req.params.newsId)
        const newsDetail = await axios.get(parliamentAPI)
        const newsById = newsDetail.data.id;
        // console.log(id)
        // // console.log(newsDetail)
        // console.log(newsById)
        res.render("news-detail", { layout: false, newsDetail });
    } catch (err) {
        next(err);
    }
});

module.exports = router;