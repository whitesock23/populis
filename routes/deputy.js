const express = require("express");
const router = express.Router();
const Deputy = require("../models/Deputy.model");
const Party = require("../models/Party.model");
const axios = require("axios");
const parliamentAPI = "https://app.parlamento.pt/webutils/docs/doc.txt?path=&fich=InformacaoBaseXIV_json.txt"

// router.get("/deputies", async(req, res) => {
//     const response = await axios.get(parliamentAPI);
//     const deputies = response.data;
//     const deputiesArray = deputies.Legislatura.Deputados.pt_ar_wsgode_objectos_DadosDeputadoSearch;
//     console.log(deputiesArray);
//     res.render("deputies/deputies-list", { deputies });
// });

router.get("/deputies-locations", async(req, res) => {
    const deputies = await Deputy.find();
    const circulosArr = []

    deputies.forEach((e) => {
        circulosArr.push(e.circulo);
    });

    // console.log(circulosArr);

    let uniqueCirculos = [...new Set(circulosArr)];
    // console.log(uniqueCirculos);
    res.render('deputies/circulos', { uniqueCirculos });
})

router.get("/deputies/:circulo", async(req, res) => {
    const theCirculo = req.params.circulo;
    // console.log(theCirculo);
    const deputies = await Deputy.find({ circulo: theCirculo })
    console.log(deputies);
    res.render("deputies/deputies-list", { deputies });
})

router.get("/parties", async(req, res) => {
    const parties = await Party.find()
    res.render("parliament/parties", { parties });
})


router.get("/deputies/:deputyId", async(req, res) => {
    const deputyDetail = await deputyAPI.findById(req.params.deputyId);
    res.render("deputies/deputy-detail", deputyDetail);
});

router.get("/deputies/:deputyId", async(req, res) => {
    const deputyBycircle = await deputyAPI.findById(req.params.deputyId);
    res.render("deputies/deputy-detail", deputyDetail);
});

module.exports = router;